...
- データ(data/*.json・data-test/*.json)の変更は、設定→開発者向け「同梱データを使う」を ON にすれば **push なしで実機検証できる**(JSON 編集のたびに `installDebug` で焼き直す)。リモート取得(GitHub raw main 優先)の経路で検証したい場合のみ実機検証の前にプッシュが必要で、その際は理由を説明して確認を取る
