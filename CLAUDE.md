...
- **設定タブも同じ list-detail 二ペイン**(#56): 分割判断・ペイン様式は #54/#55 と共通(グローバル TopAppBar なし・一覧ペイン先頭に `PaneHeader`「設定」・選択行 `secondaryContainer` ハイライト・Rail/下部ナビ維持・destination は UiState から算出)。左=カテゴリ行(現在値サマリ付き)、右=サブページ内容で、未選択のときは右ペインにプレースホルダを出す(先頭カテゴリの自動選択はしない——タブに来ただけで 1 つ選ばれた状態にせず、戻る操作の起点もぶらさないため)。1階層目のサブページは右端✕で選択解除、**2階層目**(オープンソースライセンス・取得した地図データ)は左端←で親カテゴリへ戻る(ペイン内置換。この間も一覧は**親カテゴリの行**をハイライトしたまま)。親子関係は `SettingsSubpage.parent` が単一の出所で、戻る操作(`onCloseSettingsSubpage`)とハイライト算出で共用する。**タブ離脱時は `onSelectTab` が `settingsSubpage` をクリアする**(二ペインではサブページを開いたまま Rail で他タブへ移れるため。残すと他タブに設定サブページが全画面で漏れる)。ペイン内容は `ListItem` が自前で 16dp を持つため `PaddedColumn` に入れず、見出し行だけ端に寄せる。縦画面は無変更(詳細は code-guide.md 6.4)。
- **詳細表示の面は表示形態で決める**(#78): 縦・一ペインの全画面オーバーレイは素の surface(面を立てない)。横の常設ペインは surfaceContainerLow——お店/おトク/設定の詳細ペインは共通の `DetailPaneSurface`(全周 16dp 角丸・画面端 16dp マージン・影なし。未選択プレースホルダも面の中に入れ、空の面で境界を常時見せる)、地図の右ペイン 320dp は画面端接地のため角丸なし。地図の上に**浮く**サイドシートだけ影 2dp(浮きは影、常設面はトーン差で表現)。一覧ペインは面を立てない(詳細だけが container の M3 canonical list-detail)。**面の上に置く `ListItem` は `transparentListItemColors()`**(UiHelpers)で containerColor を透明にする(既定の surface が面を打ち消して白く浮く。縦は background==surface のため見た目不変)。面の入れ子は上位ロール+1 段小さい角丸(`SettingsGroupSurface`=surfaceContainerHigh・12dp。ペインと 1 段差では実機で読めず 2 段差にしている)。詳細は code-guide.md 6.4。
