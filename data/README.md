...
- `municipalities.json` — **未作成(Phase E で作成予定)**。全国自治体マスタ。設定画面で居住地・行動圏の自治体を登録する際のピッカーデータとして使う。**総務省の全国地方公共団体コード等のオープンデータから生成する方針**。アプリ固有のロジック(グループ分け等)は UI コード側で扱い、マスタ自体はオープンデータの構造を活かす。
...
- `prefectures[]` — 47 都道府県の配列
- 各都道府県は `municipalities[]` (市区町村のフラット配列) または `groups[]` (グループ分けされた配列) を持つ
- 東京都のみ `groups` を使い「23区」「市部」「町村部」に分ける
- 各市区町村: `{ name, type }` — type は `"city"` / `"ward"` / `"town"` / `"village"`
