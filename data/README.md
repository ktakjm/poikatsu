...
- 収録するのは**決済分**(カード決済・iD 等、支払いが条件の特典)のみ。**提示分**(ポイントカード提示・カード現物提示が条件)は必ず決済分と分離して起こし、スキーマの受け皿(#80 カード現物提示 / #39 ポイントプログラム会員提示)ができるまで理由付きで見送りに回す。閾値 2% は提示施策にも同じく適用する(全加盟店共通の通常提示分 1% は閾値未満かつ自明なので収録しない)。詳細は collect-campaigns の mapping.md「提示と決済の分離」
