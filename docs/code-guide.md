...
- **距離は起点基準（GPS or 地名検索地点）**: リストの距離表示・並び順は `loadNearbyAround` の `originLat/originLon` から測る。GPS 起点（既定）なら距離は現在地（`userLat/userLon`）基準、地名起点なら検索地点（`nearbyOrigin.lat/lon`）基準になる。「このエリアを検索」は地図中心で YOLP を引き直すが距離の起点は変えない。`NearbyUi.userLat/userLon` は常に実 GPS（青ドット用）を保持し、距離の意味とは分離する。`centerLat/centerLon` は YOLP の検索範囲・地図カメラ・「このエリアを検索」表示条件の基準に使う。
