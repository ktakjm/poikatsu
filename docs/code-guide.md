...
└── app/src/
    ...
    └── test/java/com/ktakjm/poikatsu/
        ...
        ├── StoreVisibilityTest.kt        # 地図 POI の表示分類（通常ピン/薄いピン+理由/描かない。#77）
...
erDiagram
    ...
    POINT_CURRENCY {
        ...
        bool value_fixed "円建てで1pt価値が固定か(au PAY残高等。設定画面に出さない。#83)"
    }
    CARD_CLASS {
        string id PK "例: w / s"
    ...
    POINT_MULTIPLIER {
        ...
        double factor "倍率(例: 1.5)。選択肢があるときは未選択時の既定=最小値"
        double_list factor_options "ユーザーが選べる倍率(任意。Pontaの交換所 1.1/1.5 等。#83)"
        string group "倍率グループ(任意。ウエル活のVポイント/WAON POINT等。ON/OFF連動。#84)"
...
- **カスタムの自治体キャンペーン（#91）**: `CustomCampaign.region: Region?` が非 null なら自治体キャンペーン（お店の指定＝`allStores` / `merchantIds` / `bannerSelections` / `storeNames` とは排他。エディタは切替で相手側を消さず、保存時に選ばれている側だけを写す）。`toCampaigns` は同梱 municipal と同じ形（`type=municipal`・`store_scope=external`・`merchant_rules` 空・`region`・`display_name` なし）に変換し、`mergeUserData` が合流直後に `applyMunicipalDefaults`（QR サービス既定文言）を掛ける——同梱は `resolveCampaigns` で読み込み時に掛かるが、カスタムはパースを通らないため合流側で個別に掛ける。以降の自治体グルーピング（`campaignGroupKey`＝`municipal:{region.name}`）・地域フィルタ・地図お知らせピル・通知は type と region しか見ないので追加実装なし。`CustomPayment` の差分（`detailUrl` / `note` / `ineligibleNote` / `pointCurrencyId`）は `payment_variants` と同じ「単値は上書き・注記は共通→決済の順で連結」で展開後 Campaign に写る（お店のキャンペーンでも同じ規則）。地域は `RegionPickerDialog`（ui/RegionPickerDialog.kt。単一選択・県全域行あり・グループなし。検索は `searchRegions`）で選び自由入力は持たない（名称不一致で地域フィルタ・ピル・通知が効かなくなるため）。同じ自治体の同梱施策と 1 カードに束なるため、施策詳細の編集・削除コールバックは `(Campaign) -> Unit` にして混在グループでは登録ごとにカード直下へ出す（下記おトクタブの節）
...
`StoreScope`（`MANAGED` / `EXTERNAL`。#86）も同型の enum だが、こちらは JSON との写像をコンストラクタ引数 `storeScopeRaw: String` に分離し、`Campaign.storeScope` 自体が enum を返す導出プロパティになっている（生値が要るのは整合性テストのバリデーションだけのため）。施策の帰属 4 種（card_id / card_brand / payment_method_id / point_program_id。「ちょうど 1 つ non-null」の排他）は `sealed interface Attribution`（`Card(id)` / `Brand(name)` / `Qr(id)` / `Program(id)`）に写す導出プロパティ `Campaign.attribution: Attribution?` があり、帰属で分岐するコード（brandColorOf・resolveCard・payoutCurrency・judge 系フィルタ・NotificationPlanner・CampaignGrouping 等）は when の網羅チェックが効くこちらを使う。カスタムキャンペーンの `CustomPayment`（cardId / qrPaymentId / cardBrand の同型 3-nullable）にも同じ `attribution` 拡張があり、エディタの選択キー・operator 解決で共用する。JSON スキーマ・生フィールドはそのまま（Campaign 構築時はシリアライズの源泉として raw フィールドに書く）。
...
近隣取得は非同期なので、**読込中に「お店」タブへ移ったのに取得完了で「地図」へ戻される**のを防ぐため、`NearbyController`（#88 で MainViewModel から分離した地図系 delegate。後述）は世代カウンタ（`nearbyGeneration`、`@Volatile`）を持つ。`fetchNearby`/`searchHere` は開始時に世代を進めて捕捉し、`onCloseNearby`（タブ移動）でも進める。取得完了時は `applyNearbyIfCurrent(gen, …)` で**捕捉した世代が最新のときだけ** `nearby` に反映し、タブを離れた後・再取得で破棄された古い結果は捨てる。
...
パーミッション要求は UI 層（`rememberLauncherForActivityResult`）の責務で、下部ナビの「地図」タブのタップを起点に確認・要求する。ViewModel 側（`NearbyController`）は「許可済み前提」の `fetchNearby()` と「拒否された」`onLocationDenied()` だけを持つ。FINE / COARSE のどちらか片方でも許可されれば検索する。
...
**コードの置き場(#88)**: 地図タブのロジック(近隣検索・位置情報・地名検索の約30メソッド+`LocationProvider`/`Geocoder`/`YolpClient` 依存)は `ui/NearbyController.kt`(MainViewModel の delegate。`viewModel.nearby.fetchNearby()` のように UI から直接呼ぶ)、地図タブの Composable(`NearbyPane`/`NearbyDetailSideSheet`/`NearbySheetContent` ほか)は `ui/NearbyScreen.kt` にある(以前はどちらも MainViewModel.kt / PoikatsuApp.kt に同居)。delegate は VM と共有の `_state` を直接更新する(専用 ViewModel に分けるとタブ切替・ブリッジ・rebuild 連動で 2 つの StateFlow の同期が要るため採らない)。地図タブの状態は `UiState.nearby: NearbyState` に 2 層で入れ子にする——検索1回ごとに作り直す層(`NearbyState.search: NearbyUi?`)と、再検索をまたいで持続する層(`searchFailed`/`selectedCategories`/`merchantFilters`/`origin`/`geocodeCandidates`/`isGeocoding`)。持続層のうち絞り込み 2 つはタブ離脱でも保持し、それ以外は離脱時に `NearbyState.cleared()` で消す。地名検索の 7 引数(originName・候補・isGeocoding+操作4つ)は `PlaceSearchState`+`PlaceSearchActions` の対に束ねて PoikatsuApp→NearbyPane→NearbyMap を素通しする。

```mermaid
sequenceDiagram
    participant U as ユーザー
    participant SC as PoikatsuApp
    participant VM as NearbyController（MainViewModel の delegate）
...
- **間引いた店の薄いピン（#77）**: 地図から店が消える経路（公式対象外 `isExcludedStore` の店舗ごと除外・網羅リスト外 `exhaustiveListIneligibleCampaignIds` の施策単位間引き・ユーザー登録ペア #63）はいずれもユーザーに見えず、「目の前の店が理由なく出ない」ように見えていた（#70 で空状態の文言・調べる導線は手当てしたが、リストが空にならない混在レンズでは無言のまま）。POI の分類は `domain/StoreVisibility.kt` の `JudgmentEngine.classifyStore(...)` に集約し（`loadNearbyAround` と `recomputeNearbyPlaces` で共有）、結果は `Shown`（通常ピン。ブリッジ中の下見残しも含む）/ `Hidden(reason)`（薄いピン）/ `Dropped(exhaustiveIneligible)`（描かない）の 3 値。**薄いピンは「除外が無ければ特典が出ていた店」だけ**——除外を外して `judgeAll` し直し、実際に施策を消した除外を理由にする（公式対象外 > 網羅リスト外 > ユーザー登録。所有カードの都合等で元々出ない店まで薄くすると「対象外」の意味がぼやけるため Dropped）。薄いピンは `NearbyUi.hiddenPlaces` に**通常ピンと別リスト**で持つ（`places`＝対象店の意味を変えず、一覧・件数バッジ・「お店で絞る」・空状態文言・`ineligibleHiddenCount` は従来のまま）。`recomputeNearbyPlaces` は `places + hiddenPlaces` を再分類するため、#63 の登録/解除で通常⇄薄いが再検索なしで双方向に動く（以前は「一度消えた店は次の検索まで戻らない」制限があった）。描画は `MapMarker.dimmed=true` → `NearbyMap` がクラスタ管理の外で `MarkerComposable`（zIndex 負）として `DimmedStorePin`（`surfaceVariant` 塗り+`outline` 縁・20dp・選択時 30dp。ブランド色は使わない）を描く。近接グルーピングも掛けない（1 店 1 ピン）。プレビュー（`NearbyPreview`）は還元の代わりに理由文（Info+onSurfaceVariant）を出し、公式対象外・網羅リスト外は「根拠を確認」→ `onOpenNearbyStoreCheck` が店舗判定画面を**単独で**開く（判定詳細は `isExcludedStore` を反映せず対象のように見えるため下に積まない。横画面のサイドシート表示中のピンタップも同じ差し替え）、ユーザー登録は「詳細を確認」→ 判定詳細の「対象外に登録済み」から解除。表示トグルは `AppSettings.showIneligibleStorePins`（設定→表示。既定 ON。OFF でも分類は行い `NearbyPane` で描画だけ抑止）。開発者向け POI 一覧の理由に `USER_EXCLUDED`（登録対象外）を追加。
...
- YOLP は支店名を `Name` に内包するため `Poi.name` がそのまま表示名になる。公式に「対象外」と明示された店舗（`isExcludedStore`）は近隣リストに出さず、地図には薄いピン（#77）で残す（照合も `name` で行う）。
...
`./gradlew :app:testDebugUnitTest` で全 527 テストが JVM 上で実行される（エミュレータ不要。件数は 2026-08-30 時点）。
...
| `BannerTest` / `NearbyTest` / `StoreVisibilityTest` / `RegionFilterTest` / `MunicipalitySearchTest` ほか | 看板照合・チェーン特定・地図 POI の表示分類（#77）・YOLP・地域フィルタ・自治体検索 | フィクスチャ+実データ |
