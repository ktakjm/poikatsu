...
- **詳細画面から戻ると地図が「少し北→少し南」へわずかに沈み込む**(2026-06-21 実機確認)。地図生成時の初期カメラ(余白なし)と、表示後に当てる `contentPadding`(=ボトムシート peek 分の下端余白。Google ロゴをシートに隠さないためのもの)の差による。ロゴ視認性とのトレードオフのため未対応。消すなら「シートの開き具合に地図 contentPadding を追従させる」「初期カメラに同じ余白を見込む」等の対応が要る。
...
- **ジャンルコードが空の店舗**(例: オーケー)は `gc` で取れず店名キーワードで個別取得している。オーケー以外にも空ジャンルのチェーンがあれば同様に取りこぼす(`YolpClient.KEYWORD_QUERIES` に追加して対応)。現状この一覧は YolpClient にハードコード。増えるようなら merchants.json 側に「キーワード検索が必要」メタデータを持たせ、データ駆動化するのが筋(data/domain の責務分離も明確になる)。
...
- **ビューポート検索の極端なズームアウト**: 「このエリアを検索」は地図の可視範囲(中心→北東角)から半径を算出するため、街全体まで縮小すると半径が大きくなり YOLP の件数上限(1 ソース最大 500 件=5 ページ)に当たって遠方の対象店を取りこぼし得る。通常の街区ズームでは問題なし。必要なら可視範囲の上限クランプや「ズームインしてください」表示で対処(現状未対応・実用上許容)。
