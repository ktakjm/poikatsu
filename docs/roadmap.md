...
- [x] **近くのお店モード: 表示する場所の検索（起点コントロール）**（2026-06-24 実装・実機確認済み）：「これから新宿に行くので新宿駅周辺を先に見たい」型の需要に応える。地図モードに**地名で起点を移す検索**を足した。位置づけ：3 層モデルの新層ではなく**「近く」モード固有の"起点（どこを見るか）"コントロールの拡張**。既存の起点操作（📍現在地で検索＝起点=実GPS／このエリアを検索＝起点=地図中心）に **3 つ目の値「任意の地名」**を足すもので、**チェーン名のテキスト検索とは別物**（「探す」は位置概念を持たないので重複せず境界はぼやけない）。
  ...
  - **ジオコーディングは Android 標準 `Geocoder`**：**依存追加なし・APIキー不要・無料・ライセンス問題なし**（フレームワーク API ゆえ licenses.md 追記も不要）。API 33+ は非同期版、29〜32 は blocking 版を別スレッドで（minSdk=29）。`Geocoder.isPresent()=false` 端末では手動パンにフォールバック。
