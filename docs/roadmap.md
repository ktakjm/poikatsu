...
- [x] **近くのお店モード: 地図ピンのクラスタリング（密集時に件数表示）**（2026-06-24 実装・実機確認済み）：`maps-compose-utils` の `Clustering` コンポーザブル（Apache-2.0・[licenses.md](licenses.md) に追記済み）を採用。密集ピンを `inverseSurface` 色の円＋件数バッジにまとめ、ズームインで個別ピン（従来のブランドカラー多色）に展開する。個別ピンの描画は `storePinDescriptor`（Canvas ビットマップ）から Compose `drawBehind` に移行し、`Clustering` の `clusterItemContent` で直接描画する。**クラスタタップ**＝現在ズーム+2（上限 `MAX_CLUSTER_ZOOM=19`）へアニメーション＋プレビュー表示中ならリスト表示に戻す（`onClusterTap`→`onClearNearbyPreview`）。**リスト行タップ時のズーム**＝クラスタ解除のため最低 `SELECTION_MIN_ZOOM=17` まで寄る（既に深ければ維持。密集商業施設で 17 でも解除できない場合はクラスタ表示のまま許容）。クラスタリングは `NearbyMap.kt` 内部に閉じ込め（`StoreClusterItem` : `ClusterItem`）、アプリ側は従来どおり `List<MapMarker>` を渡すだけ。フィルタ（ジャンル/チェーン）適用後の `visiblePlaces` がクラスタの入力＝フィルタ変更でクラスタ件数もリアルタイムに追従する。現在地マーカー（青ドット）はクラスタ対象外。
