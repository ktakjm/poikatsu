# poikatsu 進捗とロードマップ

開発の現在地と今後の計画をまとめるドキュメント。フェーズの定義と背景は [PLAN.md](../PLAN.md) を参照。
コードの構成は [docs/code-guide.md](code-guide.md) を参照。

最終更新: 2026-06-30

## 1. 現在地サマリ

**Phase 1（MVP）は完了**（2026-06-12）。Phase 2（店舗単位判定・GPS 周辺検索・期間限定キャンペーン・自治体施策・QR 決済クーポン・設定画面拡張）も完了（2026-06-30）。Phase F のデータ投入が済み実機検証待ち。

```mermaid
flowchart LR
    P1["Phase 1: MVP<br/>検索→判定表示<br/>リモートデータ化"] --> P2["Phase 2: 拡張<br/>店舗単位判定 / GPS /<br/>自治体施策 / QRクーポン"] --> P3["Phase 3: 最適化<br/>期待価値スコア<br/>ウエル活・期限ポイント"]

    style P1 fill:#2E7D32,stroke:#1B5E20,color:#fff
    style P2 fill:#2E7D32,stroke:#1B5E20,color:#fff
    style P3 fill:#E0E0E0,stroke:#9E9E9E,color:#333
```

| フェーズ | 状態 |
|---|---|
| Phase 1（MVP） | ✅ 完了（2026-06-12） |
| Phase 2（拡張 + キャンペーン Phase A〜F） | ✅ 完了（2026-06-30）。実機検証待ち |
| Phase 3 | ⬜ 未着手 |

## 2. 完了した作業

### Phase 1 マイルストーン（すべて完了）

| マイルストーン | 内容 | 実績メモ |
|---|---|---|
| M1: データ作成 | 三井住友・MUFG の対象店舗を公式ページから手動収集 | merchants 59 チェーン / 2 施策・計 62 merchant_rules。reading・aliases・brand_color 付き |
| M2: アプリ骨格 | Compose プロジェクト + JSON 読み込み | Room は不要と判断し見送り（ファイルキャッシュで代替）。`data/` を assets として直接同梱する構成に |
| M3: 検索と判定表示 | インクリメンタル検索 + 判定カード | エイリアス・ひらがな/カタカナ正規化・前方一致優先。計画外の追加: カテゴリ複数選択フィルタ、ブランドカラー表示 |
| M4: リモートデータ化 | GitHub raw フェッチ + キャッシュ | データ更新はリポジトリの JSON 編集のみでアプリ再ビルド不要に |

**Phase 1 完了の定義を達成**: 「サイゼリヤ」と入力すると「三井住友カード スマホのタッチ決済で 7%」が即表示される。

### Phase 1 完了後の追加実装（コミット履歴順）

| コミット | 内容 | 対応する計画項目 |
|---|---|---|
| `b6ad172` | フォアグラウンド復帰時の自動更新（1 時間間引き）+ 手動更新ボタン + データ鮮度表示 | 計画外の運用改善 |
| `568b38b` ほか | 店舗単位の対象判定。当初は ⛔ 公式対象外パターン / ⚠ 商業施設内リスクの 2 段階警告だったが、⚠ キーワード警告は実際の対象外店舗との乖離が大きくノイズのため廃止。**公式が対象/対象外を言い切っているチェーン（`official_store_list`）に限り、別画面で店舗名を入力し対象/対象外を断定表示する方式に変更**（公式情報の更新日を併記）。 | **Phase 2-1 完了（方針変更済み）** |
| `a85be2f` | GPS 周辺店舗検索（Overpass API / OSM、半径 500m〜3km、対象チェーンのみ距離順表示 → タップで店舗名引き継ぎ判定） | **Phase 2-4 完了**（Google Places を使わず OSM のみで実現） |
| `744d3f1` `d7700a8` | 近くのお店モードの UI 改善: 地図のダークモード追従（タイルに `INVERT_COLORS`）＋店舗一覧を引き上げ式ボトムシート化（地図を全面表示） | **3.3 フォローアップ完了**（GPS 地図のUI改善） |

### 基盤・運用面で整備済みのもの

- ユニットテスト 43 件（実データを使った検索・判定・データ整合性チェック含む）— `./gradlew :app:testDebugUnitTest`
- ライセンス管理ルールと調査記録（[licenses.md](licenses.md)）
- データスキーマ仕様と月次更新ルール（[data/README.md](../data/README.md)）
- ユーザー固有前提の分離（profile.json）— 将来の設定画面はこのファイル構造をそのまま編集対象にできる

## 3. 今後のロードマップ

### Phase 2 完了済み

```mermaid
flowchart TD
    subgraph done["完了済み"]
        T1["✅ 個別店舗の対象判定<br/>（official_store_list / 公式断定リスト）"]
        T4["✅ GPS 店舗候補<br/>（YOLP + Google Maps）"]
        T2["✅ 自治体還元施策<br/>（キャンペーン Phase A〜F）"]
        T3["✅ QR 決済クーポン導線<br/>（キャンペーン Phase A〜F）"]
    end
    T1 --> T2
    T2 --> T3

    style T1 fill:#2E7D32,stroke:#1B5E20,color:#fff
    style T4 fill:#2E7D32,stroke:#1B5E20,color:#fff
    style T2 fill:#2E7D32,stroke:#1B5E20,color:#fff
    style T3 fill:#2E7D32,stroke:#1B5E20,color:#fff
```

#### 3.1 自治体還元施策（Phase 2-2）✅ 完了

キャンペーン Phase A〜E で実装済み。`type: "municipal"` / `store_scope: "external"` / `region` フィールド / 設定画面の自治体登録 / キャンペーンタブでの一覧表示 / 期間フィルタ。

#### 3.2 QR 決済クーポン導線（Phase 2-3）✅ 完了

キャンペーン Phase A〜E で実装済み。`benefit_type`（rebate/coupon_percent/coupon_fixed）/ QR 決済の利用登録 / 判定画面での QR セクション表示 / 外部アプリ誘導。

#### 3.3 UI 刷新：ナビゲーション整理・GPS 地図・設定画面

名前検索と GPS 検索の利用動線を整理するための UI 改善。**GPS 地図表示（2026-06-16）と、両モードを下部ナビ（`NavigationBar`）で分離するナビゲーション整理（2026-06-18）は実装済み**。下位画面（判定詳細・店舗判定）の遷移は引き続き `UiState` のフィールドで排他表現する（[code-guide.md](code-guide.md) 6.1 参照）。**設定画面は入口（歯車→全画面オーバーレイ）を新設済み（2026-06-18）**。中身は表示・データの最小のみで、テーマ／マイカード編集は後続。

- [x] **メニュー（ナビゲーション）の導入**（2026-06-18 実装）：名前検索（「対象チェーン店」）と GPS 検索（「近く」）を下部ナビ（`NavigationBar`）の対等な 2 タブに分離。従来は検索ボックス内の地図ピンアイコンで「近く」へ遷移しており直感的でなかったため、下部ナビへ移し検索ボックスのピンは撤去。Navigation Compose は導入せず、既存の `UiState` 単純状態機械を維持（`nearby == null / != null` を選択中タブに流用し、判定詳細・店舗判定はその上のオーバーレイ＝戻ると元タブへ復帰）。下部ナビは標準 `NavigationBar` の内容高 80dp を 64dp に詰めて使用（`ShortNavigationBar` は M3 Expressive 系のため不採用）。「近く」読込中にタブ移動しても取得完了で戻されないよう世代カウンタ（`nearbyGeneration`）で進行中取得を無効化。ルール/背景は CLAUDE.md「UI・デザイン方針」と code-guide.md 6.1/6.4/7.1 に記録。設定画面は引き続き未着手。
- [x] **GPS 検索の地図表示**（2026-06-16 実装）：近隣検索を地図 + 距離順リストの上下分割に変更し、対象店舗をピン表示。
  - 地図ライブラリは **osmdroid（Apache-2.0）** を採用。Google Maps SDK の地図表示は無料無制限だが Play Services 依存＋API キー＋請求先（クレカ）登録が必要で、本プロジェクトの Play Services 非依存方針と相反するため見送り（費用での脱落ではなく方針整合での判断。詳細は licenses.md）。
  - **将来 Google Maps へ「表示層だけ」低コストで差し替えられるよう薄い抽象化を導入**：地図ライブラリ固有の型を `ui/NearbyMap.kt` に閉じ込め、アプリ側は自前の `MapPoint`/`MapMarker` だけを扱う。差し替え時に触るのは NearbyMap 本体・依存・API キー設定・docs のみ（ViewModel/テストは無変更）。
  - 店舗データは **Overpass(OSM) を維持**（Places API は従量課金＋大改修のため対象外）。ピンは `brand_color` で着色（ロゴ不使用方針と整合）。明示的対象外店舗（`isExcludedStore`）は地図にも出さない。
  - 公開時の留意: osmdroid 既定の OSM 公式タイルは一般配布で利用不可。Play Store 公開時はタイル提供元の差し替えが必要（licenses.md に記載）。
- [x] **近くのお店モード: 現在地へ戻すボタン**（GPS 地図のフォローアップ・2026-06-18 実装）：地図をパン/「このエリアを検索」した後、ワンタップで地図中心を GPS 現在位置（青ドット）に戻す導線。地図右上に `FilledTonalIconButton`（`LocationOn` アイコン）を置き、タップで `mapView.controller.animateTo(userLocation)` により**カメラだけ**現在地へ寄せる（ズーム維持・通信なし・再検索しない）。周辺を取り直したいときは現在地中心になった状態で既存の「このエリアを検索」を押す（既存の「再読み込み」は GPS 取り直し＋現在地周辺で再検索する挙動のため、役割が重複しないよう本ボタンは即時のカメラ移動に限定）。**※2026-06-19、このボタンは「現在地で検索」へ役割変更し（カメラ移動だけ→現在地で再検索）、重複していた「再読み込み」を撤去した（下記「地図操作の役割整理＋再検索の体験改善」）。**
- [x] **近くのお店モード: リスト選択と地図位置の連動**（GPS 地図のフォローアップ・方式検討から）：店舗リストの行をタップしたとき、店舗情報を表示しつつ「その店舗が地図上のどこにあるか」も分かる UI にしたい。現状はリスト行タップ＝即 `onSelectNearby` で判定詳細へ全画面遷移するため、選択店舗の地図上の位置が見えない。実装方式が複数あり得る（例: 選択ピンのハイライト＋地図のセンタリング／ボトムシートを半展開のまま店舗情報をプレビュー表示して地図を残す／地図とリストの選択状態を双方向に同期）ため、**UI 方式の比較検討から**着手する。`NearbyPlace.lat/lon` は既に UI まで来ているので、主作業は地図側の「選択中マーカー」状態管理とボトムシート連携。（2026-06-17 実装）：地図を全面表示にし、店舗一覧を Material3 `BottomSheetScaffold` の引き上げ式ボトムシートへ移動（`PartiallyExpanded`・peek 200dp、展開でフルスクロール）。地図を広く見せつつ、必要時に一覧を引き上げて確認できる。半径チップとヘッダーもシート/`topBar` に再配置。**（2026-06-18 実装・方式は「プレビューシート」を採用）**：行/ピンのタップを全画面遷移から「選択」に変更。`NearbyUi.selectedPlace` を新設し、選択中はボトムシートを店舗プレビュー（店名・距離・カテゴリ・最大還元率・「判定の詳細を見る →」）に切り替える。地図はその店へ `animateTo` でセンタリングし、ピンを拡大＋白縁強調して最前面に描く（`MapMarker.selected`）。判定詳細へはプレビューのボタンから明示遷移し、× / 戻る で一覧に復帰。行とピンのどちらからでも選択でき、選択状態は双方向に反映される。再検索（`searchHere`/半径変更/`fetchNearby`）では新しい `NearbyUi` で `selectedPlace` が null に戻り選択が解除される。
- [x] **近くのお店モード: 地図のダークモード追従**（2026-06-17 実装 / 2026-06-19 改善）：OSM ラスタタイルは描画済み画像でテーマに反応しないため、表示が暗いとき osmdroid の `TilesOverlay.INVERT_COLORS` をタイルに適用（明るいなら解除）。当初は `isSystemInDarkTheme()`（OS 設定）で判定していたが、設定画面のテーマ上書き（システム/ライト/ダーク）に追従しなかったため、`MaterialTheme.colorScheme.surface.luminance()` で「実際の表示が暗いか」を見る方式に変更。本格的なダーク配色が必要になれば専用ダークタイル（要・規約/帰属確認）への差し替えが次の選択肢。
- [x] **Material 3 追従のデザイン改善**（2026-06-18 実装）：(1) テーマを dynamic color 中心＋11 以下は M3 ベースラインに刷新（Android Studio テンプレ紫を撤去、`Color.kt` 削除）、ベーステーマを DayNight 化（ダーク起動時の白フラッシュ解消）。(2) 全画面を単一 `Scaffold` + `TopAppBar` 化し手書き Row ヘッダーを全廃。(3) 状態表現を絵文字→Material アイコン＋セマンティックカラー、警告色を `warningColor()`（独自 warning ロール）に分離、ブランド色上の文字は `onColorFor()` で可読性確保、タッチ領域 48dp、再取得失敗を Snackbar 化、行ごとの全幅 Divider を撤去。**実装ルールは CLAUDE.md「UI・デザイン方針」、背景は code-guide.md 6.4 に記録**（今後の改修もこれに従う）。
- [x] **近くのお店モード: 地図ピン・遷移先・距離の改善**（2026-06-18 実装）：(1) ピンを発行体ごとに色分け（三井住友＝緑 / MUFG＝赤）し、両対応店舗は斜めの二色ピンで描き分け（自前 `Drawable` の `storePinDrawable`。osmdroid のタップ判定のため `getIntrinsicWidth/Height` を返す）。(2) 地図ピン/リストからの遷移先タイトルに支店名を表示（`Selection.displayName`＝リストに出ている POI 名と一致）。(3) リストの距離表示・並び順を地図中心ではなく**常に現在地基準**に統一（「このエリアを検索」で遠方を見ても現在地からの距離になる）。詳細は code-guide.md 7.1。
- [x] **設定画面の入口を新設（軽スコープ）**（2026-06-18 実装）：「対象チェーン店」「近く」両モードの `TopAppBar` 右肩に歯車を置き、タップで全画面の設定オーバーレイ（`UiState.showSettings`・`onOpenSettings`/`onCloseSettings`）を開く。下位画面と同じく開いている間は下部ナビを隠し、閉じると元モードへ復帰。中身は現状「データ」（鮮度表示＋今すぐ更新）のみで、表示（テーマ／dynamic color）とマイカード（profile.json の編集＝書込み可能な DataStore 移行が前提）は後続ステップ。あわせて「近く」モードの**再読み込みボタンを半径チップ行の右端へ移設**（地図表示時はチップごとボトムシート内に入る）し、歯車と更新の同居を解消。更新導線自体の整理（後述バックログ）は別途。
- [x] **近くのお店モード: 地図描画・店舗データソースの刷新（Google Maps + YOLP）**（2026-06-20 方針決定 / 2026-06-21 実装・実機確認済み）：OSM（osmdroid 描画 + Overpass データ）は新規開店店舗の欠落・支店名（「○○店」）の欠落・地図デザインの質で実用に劣るため、**描画を Google Maps SDK、店舗データを YOLP ローカルサーチAPI へ移行**する。**上記 3.3 の「Play Services 非依存で osmdroid を採用」「Overpass を維持」判断を意図的に転換**するもの（理由・規約・フェーズ戦略の全文は [map-data-stack.md](map-data-stack.md)）。要点：YOLP は地図描画系を 2020 年に廃止済みのためデータ系のみ使用／**YOLP データはキャッシュ禁止**（現状の「POI 永続化なし」を維持）・全体 5 万リクエスト/日・**アプリ下部に Yahoo クレジット必須**・無料配布が前提／Google Maps は **Play Services + 課金アカウント + API キー（パッケージ名+SHA-1 制限・`BuildConfig` 経由で非コミット）** が必須。フォールバック（5万到達時に同日 OSM）は当面作らず、公開・スケール時に YOLP Premier / フォールバック / オール Google を再評価。描画層は `NearbyMap.kt` 隔離を維持し将来 MapLibre へ差し替え可能なまま残す。**コア完了**（描画ダーク純正配色・YOLP gc=01/0205＋キーワード・支店名重複排除・施設テナント除外・クレジット表示まで実機確認済み）。残る軽微な継続課題（~~詳細復帰時の微小なカメラ移動~~〔2026-06-23 解消・上記 full-bleed 項〕、支店名違いの重複残り、空ジャンル店舗・短いチェーン名の取りこぼし、テナント除外 heuristic の限界、`KEYWORD_QUERIES` のデータ駆動化）は [map-data-stack.md](map-data-stack.md) §8、スケール時の選択肢（Premier / フォールバック / オール Google）は同 §6 に集約。
- [x] **近くのお店モード: 検索範囲のビューポート化＋ボトムシート圧縮**（2026-06-22 実装・実機確認済み）：半径チップ（500m/1km/3km）による検索範囲指定を廃止し、**Google Maps 同様「画面に写っている範囲を検索」**へ変更。「このエリアを検索」タップ時に地図の可視範囲（中心→北東角の距離）から半径を算出し、その時のズームを `NearbyUi.zoom` に記録して結果反映時もカメラを動かさない（ズームが勝手に戻らない）。`YolpClient` は半径(`dist`)方式のまま無改修で、UI 側が可視範囲→半径を渡す。初回・「現在地で検索」・地名検索は既定半径 2km で YOLP に問い合わせ、500m 以内の店舗が 10 件未満ならズーム 15（広域）、10 件以上なら 16（標準）に適応する。**3.3 の「半径チップ 500m〜3km」前提を意図的に転換**するもの（ズーム=検索範囲の方が直感的・コントロールを1つ減らせる・ボトムシートの狭さ解消に最も効く）。トレードオフ：街全体まで極端にズームアウトすると YOLP 件数上限（1ソース500件）で遠方を取りこぼす可能性（通常ズームでは問題なし。[map-data-stack.md](map-data-stack.md) §8 の取りこぼし課題に連なる）。あわせて**ボトムシートを圧縮**：下部ナビ 64→56dp、ドラッグハンドル小型化（既定は上下余白が厚くクレジットが間延び）、peek 200→220dp、プレビューのボタン下に余白（「詳細を確認」が下端で欠けるのを解消）。
- [x] **近くのお店モード: 絞り込みの整理（3 層モデル：モード / レンズ / ブリッジ）**（2026-06-22 (1)〜(3) 実装・実機確認済み）：「近く」に機能を足していくと散らかるのを防ぐため、操作を性質で 3 層に分けて設計する。**モード**＝問いの種類（下部ナビ。「探す＝名前起点」「近く＝現在地起点」の 2 つで固定し増やさない）。**レンズ**＝表示中の集合を絞る加算操作（フィルタ。語彙を 1 つに統一し両モードで共有）。**ブリッジ**＝モード間で選択を運ぶ一方向リンク。背景：名前検索と GPS 検索は別の問いなので 2 モードに分離済み（3.3）。ここに**生のチェーン名テキスト検索**を足すと境界がぼやけるため、**検索の入口（テキスト）は「探す」に 1 つだけ**とし「近く」では型入力させない。この 3 層を崩さない範囲で以下を実装する。
- [x] **近くのお店モード: ズームアウト時の密度偏り是正（gc 絞り込み＋クリップ）**（2026-06-25 実装・要実機確認）：上記ビューポート検索のトレードオフ（街全体までズームアウトすると密な業種が 500 件上限＋近い順で中心に打ち切られ、疎なキーワードチェーンだけ周縁に残って「周縁がカーブスばかり」になる）への対策。(a) `YolpClient.mergeAndClip` が**上限到達ソースの最遠距離の最小値**を共通カバー半径として全ソースを切り捨て密度を揃える。(b) グルメ gc を過密な `01`（新宿駅 3km で 8459 件）から対象業種だけの **`gc="0123,0115,0101013"`（カンマ OR・1 コール）** に絞り約 1870 件へ。密集地でクリップ半径が潰れて検知数が激減する退行を**コール数ほぼ不変**で解消。あわせて gc が空/バラつく上島珈琲・はま寿司・ピザハットを `KEYWORD_QUERIES` に追加（gc ソースとの重複は座標+名前一致＋ViewModel `distinctBy` の二段で 1 件化）。実 API 検証メモ：`gc` はカンマ OR 可（スペース区切り不可）、`query` は OR 不可だが別名辞書で表記揺れに強い（KFC=ケンタッキー=ｹﾝﾀｯｷｰ）。詳細は [map-data-stack.md](map-data-stack.md) §8・[code-guide.md](code-guide.md) §7。これにより 111 の「YOLP gc=01/0205」は現行 `gc=0123,0115,0101013 / 0205` に更新。
  - [x] **(1) ジャンル絞り込み（レンズ・最優先）**（2026-06-22 実装・実機確認済み）：「探す」のカテゴリ複数選択（`FilterChip` + `engine.categories`）を「近く」に展開。新機能ではなく絞り込み語彙の統一。`nearby.places` を派生フィルタした `visiblePlaces` を地図 `markers`・一覧の両方に適用（クライアント側・YOLP 再フェッチ不要）。フィルタ状態は再検索（`searchHere`/`fetchNearby`）をまたいで保持するため、毎回作り直す `NearbyUi` ではなく `UiState.nearbySelectedCategories`（探す側 `selectedCategories` とは独立）に置く。配置はボトムシート内に横スクロール1行（`NearbyCategoryChips`）。空表示は「範囲に無い」と「絞り込みで0件」を出し分け。
  - [x] **(2) チェーンで絞り込む（レンズの 2 段目）**（2026-06-22 実装）：「ファミレスで絞ったら候補が多い → どのチェーンがあるか見てサイゼに絞る」流れを切り捨てず満たす。ただし**テキスト検索は足さず**、「いま周辺に在るチェーンの一覧から選ぶ」選択式にする（＝レンズ層に留め検索層を増やさない）。一覧は **全体ではなく周辺に在るチェーン**を採用（`nearby.places` の distinct merchant、件数付き「サイゼリヤ 3 / ガスト 1 …」）。理由：(a)「近く」は周辺に在るものだけを見せる約束、(b) `nearby.places` から自動導出でデータ追加不要、(c) 全体から探したいのは「探す」の仕事＝ブリッジで対応。周辺に無いチェーンは一覧に出ない＝「周辺に無い」がそのまま答え（疑わしければ半径を広げる）。UI：常時 2 行チップは重いので「チェーンで絞る ▾」から在チェーンのピッカー（ボトムシート/メニュー）を開いて選ぶ。選択中は「サイゼリヤ ✕」の解除可能ピルを出し、解除でジャンル絞り込みへ戻る。
  - [x] **(3) ブリッジ：探す→近く**（2026-06-22 実装）：判定詳細（`JudgmentDetail`）に「近くのこの店を探す」を置き、そのチェーンに絞った状態で「近く」を開く。「特定チェーンが近くにあるか」という需要の正答。**(2) と着地状態を共有**：ブリッジも在チェーン選択も、結果は同じ「近くを 1 チェーンに絞った状態（解除可能ピル）」へ収束＝実装は単一の `UiState.nearbyMerchantFilter`（`Merchant?`）で表す。これにより検索ボックスを増やさず「名前 × 位置」を満たす。実装：判定詳細が名前検索由来（`displayName == null`）のときだけ表示し、`onFindNearby` で merchantFilter 設定＋ジャンルクリア後に `onNearbyClick`（位置情報パーミッション→`fetchNearby`）。近隣由来（地図上に既にいる）では出さない。逆方向（近く→探す）は作らない。
  - 逆方向（近く→探す）は「近く」が眺めるモードで名前を打つ動機が薄いため作らない（本数を絞る）。
  - 実装メモ：フィルタは 2 次元（ジャンル `Set<String>` / チェーン `merchantId?`）。チェーン指定時はジャンルより優先（チェーンは 1 ジャンルに属すため、チェーン選択でジャンル選択は実質無効化）。表示集合は `nearby.places` を派生フィルタした `visiblePlaces` を list・pin 双方に使う。空時メッセージは「選択中の条件に該当する周辺店舗がありません」。方式確定後は CLAUDE.md「UI・デザイン方針」と code-guide.md 7.1 にもルールを昇格させる。
- [x] **近くのお店モード: 地図の full-bleed 化＋カメラ/シートの体験修正**（2026-06-23 実装・実機確認済み）：地図系アプリの定石どおり地図モードのタイトルバー（旧 `NearbyTopBar`「近くのお店」＋歯車）を撤去し、**地図をステータスバー裏まで全面表示（full-bleed）**。設定の歯車は地図左上の浮きボタンへ移し、上端は 左＝設定 / 中央＝このエリアを検索 / 右＝現在地 の 3 点を `topInset + 8dp` で揃えて配置（スクリムは敷かず、ステータスバーアイコンの明暗は `MainActivity` で**アプリのテーマ**に追従＝システムのダーク設定でなく地図の明暗に合わせる）。ロード/エラー画面も見出しを出さず内容を中央寄せ（`Centered`）に統一。あわせて 3 つの不具合を修正：(1) **詳細から戻ると地図が「ピンの北→じわり南下」へドリフト**——初期カメラを `selectedPoint ?: center` に固定しつつ、各 `LaunchedEffect` が自分の初回だけを個別フラグでスキップ（共有フラグ方式は dispatcher の実行順で初回スキップが効かず `move(center)`→`animate(pin)` が両方走っていた）。(2) **戻るとボトムシートが沈み「詳細を確認」下端が欠ける**——既に `PartiallyExpanded` のときは `partialExpand()` を呼ばない（再生成直後のレイアウト確定前の状態変更との競合回避）＋プレビュー peek を内容実測で `maxOf(220dp, 実測)`。(3) **遷移中ローディングに見出し・歯車が中途半端に出る**を解消。詳細は code-guide.md 6.4/7.1、CLAUDE.md「UI・デザイン方針」。**これで [map-data-stack.md](map-data-stack.md) §8 / 下記 YOLP 項の継続課題「詳細復帰時の微小なカメラ移動」は解消。**
- [x] **近くのお店モード: 表示する場所の検索（起点コントロール）**（2026-06-24 実装・実機確認済み）：「これから新宿に行くので新宿駅周辺を先に見たい」型の需要に応える。地図モードに**地名で起点を移す検索**を足した。位置づけ：3 層モデルの新層ではなく**「近く」モード固有の"起点（どこを見るか）"コントロールの拡張**。既存の起点操作（📍現在地で検索＝起点=実GPS／このエリアを検索＝起点=地図中心）に **3 つ目の値「任意の地名」**を足すもので、**チェーン名のテキスト検索とは別物**（「探す」は位置概念を持たないので重複せず境界はぼやけない）。
  - **距離の基準**: 地名検索は**検索地を"仮の起点"にして距離・並び順も再計算**する（「新宿にいるつもりで見る」）。青ドット（実GPS）は参考として残す。起点は `UiState.nearbyOrigin: GeocodedPlace?`（null=GPS）に持ち、再検索をまたいで保持。「現在地で検索（📍）」で起点を GPS に戻す。検索バーの✕は**起点を GPS に戻しつつカメラは動かさない**（距離を現在地基準で再計算するが YOLP 再取得なし＝`onClearOrigin`）。**GPS 起点のとき（普段）は従来挙動と同一＝退行なし**。
  - **ジオコーディングは Android 標準 `Geocoder`**：**依存追加なし・APIキー不要・無料・ライセンス問題なし**（フレームワーク API ゆえ licenses.md 追記も不要）。API 33+ は非同期版、29〜32 は blocking 版を別スレッドで（minSdk=29）。`Geocoder.isPresent()=false` 端末では手動パンにフォールバック。`Geocoder` は住所・地名検索が主で POI（駅名等）に弱いため、**クエリが「駅」で終わらない場合は「{クエリ}駅」でも追加検索してマージ**する（「新宿」→「新宿」「新宿駅」の両方が候補に出る。座標重複は除去）。
  - **UI**: 地図上部の浮きコントロールを再構成。検索バー（`PlaceSearchBar`）＋歯車を上部1行に、📍を右下に移動。「このエリアを検索」はパンで画面の約4割以上移動 or ズームアウトで表示範囲が倍以上になったときだけ検索バー下に条件表示（虫眼鏡アイコン削除）。地名入力→候補5件→選択でカメラ移動＋再検索。検索バーは起点設定時「○○周辺 ✕」表示。
  - **レンズと直交合成**：起点（どこ）とレンズ（ジャンル/チェーン絞り込み）は別軸なので「新宿のファミレス」「新宿のサイゼ」が自然に成立。CLAUDE.md「UI・デザイン方針」と code-guide.md 7.1 に追記済み。
- [x] **近くのお店モード: 地図ピンのクラスタリング（密集時に件数表示）**（2026-06-24 実装・実機確認済み）：`maps-compose-utils` の `Clustering` コンポーザブル（Apache-2.0・[licenses.md](licenses.md) に追記済み）を採用。密集ピンを `inverseSurface` 色の円＋件数バッジにまとめ、ズームインで個別ピン（従来のブランドカラー多色）に展開する。個別ピンの描画は `storePinDescriptor`（Canvas ビットマップ）から Compose `drawBehind` に移行し、`Clustering` の `clusterItemContent` で直接描画する。**クラスタタップ**＝現在ズーム+2（上限 `MAX_CLUSTER_ZOOM=19`）へアニメーション後、「このエリアを検索」相当の YOLP 再検索を自動実行してリストをズーム後の表示エリアに更新＋プレビュー表示中ならリスト表示に戻す。**リスト行タップ時のズーム**＝クラスタ解除のため最低 `SELECTION_MIN_ZOOM=17` まで寄る（既に深ければ維持。密集商業施設で 17 でも解除できない場合はクラスタ表示のまま許容）。クラスタリングは `NearbyMap.kt` 内部に閉じ込め（`StoreClusterItem` : `ClusterItem`）、アプリ側は従来どおり `List<MapMarker>` を渡すだけ。フィルタ（ジャンル/チェーン）適用後の `visiblePlaces` がクラスタの入力＝フィルタ変更でクラスタ件数もリアルタイムに追従する。現在地マーカー（青ドット）はクラスタ対象外。**同一地点の重なり対策**（2026-06-26 実装）：同一ビル 1F/2F 等の座標が 5m 以内の店舗を `groupByProximity` で事前グルーピングし、代表マーカー（`MapMarker.groupSize > 1`）としてクラスタバッジと同じ見た目で描画。タップ時はズームではなく BottomSheet にグループ内店舗リスト（「同じ場所に N 件」）を表示し、選択で個別プレビューへ遷移する。ライブラリの `Clustering` は無改修のまま、座標前処理で閾値変更と同等の体験を実現。
- [x] **近くのお店モード: リストのソート順・距離ラベルの改善＋ボトムシート展開上限**（2026-06-25 実装）：(1) リストのソート順を起点（GPS/地名検索地点）基準から**地図中心（検索時のカメラ中心）基準**に変更（`NearbyPlace.distanceFromCenter`）。地図に映っているエリアの店が上に来る。距離ラベル（表示用）は起点基準のまま維持し、GPS 起点なら「現在地から○○m」、地名起点なら「{起点名}から○○m」を表示（起点名は先頭の都道府県名を除去し 10 文字超は省略）。(2) ボトムシートの展開上限を `heightIn(max = parentMaxHeight - topInset - 16dp)` で制限し、引き上げると検索バーを覆いつつステータスバーにはかからない位置で止まるようにした（`BoxWithConstraints` の実レイアウト高さを使用）。(3) `LocationProvider` の測位プロバイダ優先順を NETWORK→GPS から **GPS→NETWORK** に変更（エミュレータの位置注入が GPS プロバイダにのみ効くため。実機でも GPS の方が精度が高い）。
- [x] **近くのお店モード: 地名検索の候補表示名改善＋距離ラベル短縮**（2026-06-26 実装・実機確認済み）：`Geocoder` の `featureName` が施設名ではなく番地（「１」「１０１６−７５」等の数字+ハイフン）や国名（「日本」）を返すケースで、無意味な表示名（「１（東京都渋谷区）」）になっていた問題を修正。番地/国名判定時は住所コンポーネント結合の `fullAddress` を表示名にフォールバックし、コンポーネントが歯抜けの場合は `getAddressLine(0)` から国名・郵便番号を除いた完全な住所を優先する（ただし `getAddressLine(0)` は末尾に POI 名を含むことがあるため、フォールバックは番地/国名判定時のみ）。あわせて距離ラベルの起点名から先頭の都道府県名（`^.{2,3}[都道府県]`）を除去し、住所が起点名のときのラベル長を短縮した。
- [x] **近くのお店モード: 距離ラベル・ジオコード候補の安定性改善**（2026-06-26〜27 実装）：(1) 距離ラベルの起点名が都道府県名のみ（例「千葉県」）のとき、都道府県除去ロジックが全消ししてしまう問題を修正（全体が都道府県名の場合はそのまま表示）。(2) 再検索（📍・このエリアを検索）中に距離ラベルの起点名が新旧の間で一瞬ちらつく問題を修正（`stableOriginName` を `loading=false` 時のみ更新するよう安定化）。(3) 地名検索に**駅サフィックス追加検索**を導入——クエリが「駅」で終わらない場合に「{クエリ}駅」でも `Geocoder` に問い合わせ結果をマージし、POI に弱い `Geocoder` の駅名カバレッジを改善（「新宿」→「新宿」「新宿駅」の両方が候補に出る。座標重複は小数第5位で丸めて除去）。
- [x] **近くのお店モード: 位置情報取得の堅牢化＋デフォルト地点フォールバック**（2026-06-27 実装）：(1) `LocationProvider` を **GPS→NETWORK の2段フォールバック**に改善——GPS を 5 秒タイムアウトで試行し、失敗したら NETWORK（Wi-Fi/基地局）を 10 秒タイムアウトで試行、各段階で `getLastKnownLocation` も確認する（従来は GPS のみ 15 秒で屋内等のタイムアウトに弱かった）。(2) 全プロバイダで位置を取得できなかった場合（位置情報サービス OFF・全タイムアウト等）や**パーミッション拒否時**の最終フォールバックとして、**新宿駅（35.6896, 139.7006）をデフォルト地点に地図を表示**しつつ Snackbar でエラーを通知するようにした（従来は全画面エラー＋再試行ボタンで地図が出なかった）。起点は `nearbyOrigin` にセットされ、距離は「新宿駅から○○m」表示。青ドット（現在地マーカー）は非表示。ユーザーは地名検索や地図パン→「このエリアを検索」で自由に移動できる。**フォールバックは地図初回起動時のみ**——地図表示後の📍（現在地で検索）での位置情報取得失敗時は、地図・一覧を維持したまま Snackbar 通知だけ行い、起点やカメラは変更しない（既存の `failNearby` と同じ挙動）。(3) `fetchNearby` 内でパーミッション有無を先に判定し、「位置情報の許可が必要です」（パーミッション拒否）と「現在地を取得できませんでした」（パーミッションはあるが取得失敗）を正しく出し分けるようにした。
- [ ] **近くのお店モード: 横画面（landscape）の UI 改善** → [#4](https://github.com/ktakjm/poikatsu/issues/4)
- [x] **設定画面の入口（受け皿）を新設**（2026-06-18・上記項目で実装）：将来の設定を集約する受け皿を用意した。残りの中身は順次追加し、下表「profile.json の設定画面化」と統合して、エントリー状況・カードブランド編集、自治体登録（3.1）、Phase 3 のポイント残高入力に加え、**ユーザーにより還元率が変わる決済方法の還元率設定**（同じ決済手段でも利用者の契約・ランク等で還元率が異なるケースを、ユーザーが自分の値として登録し判定に反映できるようにする）を載せる。
- [x] **設定の中身（軽スコープ）を実装**（2026-06-19 実装）：**DataStore Preferences**（`SettingsRepository`）を採用し設定を永続化。(1) 表示＝テーマ3択（システム/ライト/ダーク・`MainActivity` で `PoikatsuTheme` に注入）＋ dynamic color トグル（Android 12+ のみ有効）。(2) マイカード＝カード所有チェック／還元率の手入力（公式アプリ表示値）／MUFG のブランド選択（Amex・Mastercard・Visa・JCB）／三井住友の「ウエル活 ×1.5」チェック（`point_multiplier` を profile.json に追加。係数・ラベルをデータ側に持たせ UI 文言をハードコードしない）。(3) データ＝自動更新トグル＋今すぐ更新。(4) このアプリ＝バージョン（`BuildConfig.VERSION_NAME`）/ GitHub リンク。ユーザー差分は DataStore に overlay 保存し、ViewModel のマージ層で profile に重ねてエンジンへ渡す（`JudgmentEngine` は純 Kotlin・実データテストを維持）。**判定エンジンを2点変更**：未所有カードの施策はスキップ／`amex_excluded`×Amex を警告→真の除外（検索・判定詳細・地図ピン/件数すべてに波及）。実データテスト追加（Amex 除外・所有絞り込み）。**reward の無いチェーンは検索・近隣に出さない**よう絞り込み、`entry_done` フラグと未エントリー警告は廃止（公式アプリ表示値の手入力が実効値のため）。OSS ライセンス一覧画面・カードの追加/削除・`rate_breakdown` 自動計算は今後。
- [x] **近くのお店モード: 地図操作の役割整理＋再検索の体験改善**（2026-06-19 実装）：地図上の操作が「更新（⟳）／現在地へ戻す（📍）／このエリアを検索」で役割が重なっていたのを、**検索の起点をどこにするかの 2 択**に整理した。(1) 📍 を「**現在地で検索**」へ格上げ（カメラ移動だけ→現在地を取り直してその周辺で再検索）、地図中心起点の「このエリアを検索」と対にし、重複していた ⟳ 再読み込みボタン（半径チップ右端）を撤去。(2) **再検索中も地図・一覧を残す**（直前の `NearbyUi` を `copy` して `loading` だけ立て、`NearbyPane` のゲートを「`center` があれば地図を出す」に変更）。全画面ローディングにせず、「このエリアを検索」を進捗ピルに差し替え 📍 は無効化。初回ロードのみ従来どおり全画面。(3) **再検索の一時失敗は Snackbar**（地図を残す。`failNearby` が `center` の有無で Snackbar／全画面エラーを出し分け、初回失敗は「再試行」付き全画面）。Snackbar は外側 `Scaffold` の host だとシート裏に隠れるため `BottomSheetScaffold` 自身の host に出す。(4) **地図をシート背面まで下端いっぱいに描画**し、ボトムシート角丸から背景が覗く見栄えを解消（下端 inset を当てず上端 `TopAppBar` 分だけ避ける）。背景・実装は code-guide.md 7.1。

### 期間限定キャンペーン対応（Phase A〜F）✅ 完了

実装の技術詳細は [code-guide.md](code-guide.md) を参照。

| フェーズ | 内容 | 状態 |
|---|---|---|
| Phase A | データ基盤（スキーマ拡張 + 判定エンジン） | ✅ 完了 |
| Phase B | YOLP 検索のデータ駆動化 | ✅ 完了 |
| Phase C | キャンペーンタブ + 4 タブナビゲーション | ✅ 完了 |
| Phase D | 検索・判定画面の期間限定対応（QR 判定・ベストオプション・期間限定バッジ） | ✅ 完了 |
| Phase E | 設定画面の拡張（QR 決済の利用登録 UI、自治体の登録 UI、DataStore への保存・ViewModel へのマージ、`data/municipalities.json` 新規作成） | ✅ 完了（2026-06-30） |
| Phase F | データ投入と実機検証 | ✅ データ投入完了・実機検証待ち（2026-06-30） |

### Phase 3: 最適化アドバイス（未着手）

判定エンジンを「還元率比較」から「期待価値スコア比較」へ拡張する。詳細は [#13](https://github.com/ktakjm/poikatsu/issues/13)。

### バックログ

個別の改善候補は [GitHub Issues](https://github.com/ktakjm/poikatsu/issues) で管理。`someday` ラベルは優先度低（必要になったら着手）。

## 4. 定常運用タスク

| タスク | 頻度 | 内容 |
|---|---|---|
| 常設施策データの確認 | 月 1 回 | `sources` の公式 URL を確認し `verified_date` を更新。改定があれば率・店舗リスト・`updated_at` を修正 |
| 期間限定キャンペーンの追加 | 月末 | 翌月の自治体施策・カード会社期間限定・クーポンを収集し campaigns.json に追加。収録基準・情報源・運用フローは [data/README.md](../data/README.md) の「期間限定キャンペーン・クーポンの運用」参照 |
| 期限切れデータの削除 | 月 1 回 | 終了後 30 日経過したキャンペーンを campaigns.json から手動削除 |
| 整合性チェック | データ更新のたび | `./gradlew :app:testDebugUnitTest`（merchant_id 参照切れ・エイリアス衝突を検出） |
| 店舗単位の対象情報の追記 | 発見ベース | 公式が対象/対象外を**言い切っている**完全なリストを見つけたら `official_store_list` に追記。例示レベルの情報は `exclusion_note` に文章で残すにとどめる |

## 5. リスクと割り切り（再掲・現状評価）

| リスク | 対応状況 |
|---|---|
| 施策情報が古くなり誤判定 | ✅ 対応済み: `verified_date` を判定画面に必ず表示 + 「最新の条件は公式で確認」注記 + データ鮮度（REMOTE/CACHE/BUNDLED）表示 |
| 対象外店舗リストの網羅が困難 | ✅ 対応済み: キーワード推測による曖昧警告は廃止。**公式が言い切っているリストがあるチェーンだけ**断定表示し、無いチェーンは `exclusion_note` の但し書きにとどめる（誤った断定を避ける） |
| クーポンの個人差 | ✅ 対応済み: 全員配布系の大型クーポンのみ手動データ化。個人配布は自動判定せず QR アプリへの確認導線で対応 |
| スクレイピング自動化の規約リスク | ✅ 手動収集を継続。月 1 回の運用ルール化済み |
