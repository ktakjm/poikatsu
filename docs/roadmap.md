...
#### 3.3 UI 刷新：ナビゲーション整理・GPS 地図・設定画面（未着手・将来）

現状は 1 画面に「名前検索」「GPS 検索」が同居し、画面遷移を `UiState` のフィールドで排他表現している（[code-guide.md](code-guide.md) 6.1 参照）。利用動線を整理するための UI 改善案。**今すぐ着手せず、方針メモとして記録**。
...
- [ ] **GPS 検索の地図表示**：現在は距離順リストのみ。可能なら地図上に対象店舗をピン表示したい。
  - 地図ライブラリの選定が要ライセンス確認。本プロジェクトは Google Play Services 非依存方針（code-guide 設計判断）かつ既に OSM/Overpass を利用しているため、**OSM 系（MapLibre GL Native = BSD、osmdroid = Apache-2.0 など）が方針と整合**しやすい。Google Maps SDK は Play Services 依存＋API キーが必要で方針と相反するため避ける方向。
  - 明示的対象外店舗を地図でも非表示にする現挙動（`isExcludedStore`）と整合させる。
