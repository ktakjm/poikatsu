...
| 対象 | ライセンス | 備考 |
|---|---|---|
| Material Icons のパスデータ(Storefront / Map / LocalOffer) | Apache-2.0 | `material-icons-extended` にしか無いアイコンを、依存追加を避けて google/material-design-icons のパスデータのみ `ui/theme/AppIcons.kt` に写した(2026-07-12)。Apache-2.0 のコード断片の同梱にあたるため、公開時のライセンス表記画面で material-design-icons を含める |
