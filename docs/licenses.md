...
| 将来のアプリ一般公開(Play Store等) | **問題なし**(ライセンス表記画面は実装済み: 設定→このアプリ→オープンソースライセンス。#48・2026-07-24) |
...
## 同梱しているサードパーティ由来のコード(依存ではない)

| 対象 | ライセンス | 備考 |
|---|---|---|
| Material Icons のパスデータ(Storefront / Map / LocalOffer) | Apache-2.0 | `material-icons-extended` にしか無いアイコンを、依存追加を避けて google/material-design-icons のパスデータのみ `ui/theme/AppIcons.kt` に写した(2026-07-12)。Apache-2.0 のコード断片の同梱にあたるため、ライセンス表示画面にカスタム定義(app/config/libraries/)で掲載済み(#48) |
...
| AboutLibraries Gradle Plugin(com.mikepenz.aboutlibraries.plugin.android) | Apache-2.0 | ビルド時にライセンスメタデータ(R.raw.aboutlibraries)を自動生成するのみ。APK には生成物(JSON)だけが入る |
