...
- ステータス: ~~レビュー待ち~~ → ~~issue 化~~（2026-07-04 完了: 対応A = [#34](https://github.com/ktakjm/poikatsu/issues/34)、対応B = [#35](https://github.com/ktakjm/poikatsu/issues/35)） → ~~対応A 実装~~（2026-07-05 完了） → ~~対応B 実装~~（2026-07-05 完了） → **全対応完了**（2026-07-06 実機検証済み。後続の設計論点は [#39](https://github.com/ktakjm/poikatsu/issues/39) point_currencies に分離）
...
- **B-1 の率の優先順位**: `rule.rateOverride ?: campaign.rateBase` を施策側の率とし、promotion はそれをカードの常設実効率より優先（施策に率が無ければカードにフォールバック）。card_program は従来どおりユーザー設定の実効率を優先。~~施策側の率を採用したときはウエル活係数・注記を適用しない（係数はカードの実効率にだけ掛かっている）~~ → **#39（2026-08-18）で原理的に置き換え**: 倍率は通貨の価値特性（point_currencies）になり、施策側の率でも払い出し通貨が解決でき倍率が有効なら掛ける（`payoutCurrency`+`boostedCampaignRate`(#13 で `effectiveValueRate` へ置換済み)。Vポイント払いの 15% はウエル活で実質 22.5%）
...
- **カードブランドのモデル再整理を B-2 に含める**（2026-07-05 追記）: 現状 `cards[].brand` が機能するのは amex_excluded 判定（Amex か否かの 2 値）のみで、カタログへの既定ブランドのハードコードはユーザー属性の混入（#34 実装後に smcc の brand 削除・設定画面カード行タイトルのブランド表示廃止で応急処置済み）。B-2 ではブランド一致マッチングが必要になるため、「カタログ=そのカード製品で選べるブランドの選択肢（単一なら固定）、実際のブランド=ユーザー設定（`CardOverride.brand`）」の形に再整理する
